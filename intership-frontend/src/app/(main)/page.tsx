  ...
  <div className="space-y-6 animate-in fade-in slide-in-from-right-8 duration-500">
    <div className="flex items-center justify-between mb-10 mt-2">
      <div className="flex items-center gap-4">
        <button onClick={onBack} className="p-2 -ml-2 hover:bg-slate-200 dark:hover:bg-zinc-800 rounded-full transition-colors">
    ...
    <div className="bg-white dark:bg-zinc-900 rounded-3xl border border-slate-100 dark:border-zinc-800 shadow-sm p-6 md:p-8 flex flex-col md:flex-row justify-between items-center gap-6 hover:shadow-md transition-all duration-300">
      ...
      <button onClick={onDownloadAll} className="bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 px-8 rounded-full flex items-center gap-2 transition-colors flex-shrink-0 shadow-sm shadow-blue-600/20">
        <Download className="w-5 h-5" /> Descargar todo
      </button>
    </div>
    <DownloadCard icon={Brain} iconBg="bg-orange-50 dark:bg-orange-950/40" iconColor="text-orange-400" btnColor="bg-orange-400 hover:bg-orange-500" title="Power Skills" description="Perfil de competencias personales, scores por habilidad y etapa de acreditación." pills={[{ text: '4 competencias', color: 'text-orange-500 bg-orange-50 dark:bg-orange-950/40 border-orange-100' }]} onClick={onDownloadAll} />
    <DownloadCard icon={BookOpen} iconBg="bg-blue-50 dark:bg-blue-950/40" iconColor="text-blue-500" btnColor="bg-blue-600 hover:bg-blue-700" title="Académico" description="PBL, retos, cursos y certificaciones, horas Key y experiencias internacionales." pills={[{ text: '2 PBLs', color: 'text-blue-500 bg-blue-50 dark:bg-blue-950/40 border-blue-100' }, { text: '1 certificación', color: 'text-blue-500 bg-blue-50 dark:bg-blue-950/40 border-blue-100' }]} onClick={onDownloadAll} />
    <DownloadCard icon={Heart} iconBg="bg-green-50 dark:bg-green-950/40" iconColor="text-green-500" btnColor="bg-green-400 hover:bg-green-500" title="WELL" description="Dimensiones WELL, créditos, actividades de bienestar y horas completadas." pills={[{ text: '7 dimensiones', color: 'text-green-500 bg-green-50 dark:bg-green-950/40 border-green-100' }, { text: '57h / 81h', color: 'text-green-500 bg-green-50 dark:bg-green-950/40 border-green-100' }]} onClick={onDownloadAll} />
    <DownloadCard icon={Plus} iconBg="bg-purple-50 dark:bg-purple-950/40" iconColor="text-purple-500" btnColor="bg-purple-500 hover:bg-purple-600" title="Extra" description="Teacher assistant, internships, investigación, Key Spark, clubes y proyectos sociales." pills={[{ text: '6 actividades', color: 'text-purple-500 bg-purple-50 dark:bg-purple-950/40 border-purple-100' }]} onClick={onDownloadAll} />
  </div>
);
...
const DownloadCard = ({ icon: Icon, iconBg, iconColor, btnColor, title, description, pills, onClick }: {
  ...
  <div className="bg-white dark:bg-zinc-900 rounded-3xl border border-slate-100 dark:border-zinc-800 shadow-sm p-6 md:p-8 flex flex-col md:flex-row justify-between items-center gap-6 hover:shadow-md transition-all duration-300">
    ...
    <button onClick={onClick} className={`w-14 h-14 rounded-full ${btnColor} text-white flex items-center justify-center flex-shrink-0 transition-colors shadow-sm`}>
